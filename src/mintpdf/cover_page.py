...
from reportlab.platypus import PageBreak, Paragraph, Spacer, Table, TableStyle  # type: ignore
...
def create_cover_page(metadata: DocumentMetadata, theme: Theme) -> List[Any]:
    ...
    if metadata.author:
        meta_rows.append(
            [Paragraph("AUTHOR", meta_label_style), Paragraph(metadata.author, meta_val_style)]
        )
    if metadata.organization:
        meta_rows.append(
            [
                Paragraph("ORGANIZATION", meta_label_style),
                Paragraph(metadata.organization, meta_val_style),
            ]
        )
    if metadata.date:
        meta_rows.append(
            [Paragraph("DATE", meta_label_style), Paragraph(metadata.date, meta_val_style)]
        )
    if metadata.version:
        meta_rows.append(
            [Paragraph("VERSION", meta_label_style), Paragraph(metadata.version, meta_val_style)]
        )
