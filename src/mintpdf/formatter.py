...
def format_text_to_flowables(
    ...
    logger.info(
        f"Formatting document structure using Template: {template.name}, Theme: {theme.name}, Font: {font_name} (safe)"
    )
                ...
                TableStyle(
                    [
                        ...
                        (
                            ...
                            (0, 0),
                            (-1, -1),
                            HexColor("#FAF5FF") if theme.name == "Purple" else HexColor("#F7FAFC"),
                        ),
                        ("PADDING", (0, 0), (-1, -1), 6),
            ...
            if img_match:
                ...
                if img_path.exists() and img_path.is_file():
                    try:
                        ...
                        flowables.append(
                            Paragraph(f"[Image Error: {alt_text} ({img_path.name})]", body_style)
                        )
                else:
                    flowables.append(
                        Paragraph(f"[Image File Not Found: {img_path_str}]", body_style)
                    )
