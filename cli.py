...
class MintCLI:
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                self._show_error_panel(f"An unexpected error occurred: {e}\nSee logs/mint_pdf.log for full details.")
                self._wait_for_key()
    ...
    def _draw_header(self) -> None:
        ...
        console.print(Panel(Align.center(banner), style="green", border_style="green", expand=False))
        console.print(f"[dim]Output Folder: {self.settings.output_dir}[/dim]\n")

    def _draw_main_menu(self) -> None:
        """Prints the list of main menu choices."""
        table = Table(show_header=False, box=None)
        table.add_row("[bold green]1.[/bold green] Create New PDF", "[dim white]Convert text, markdown, or docx into styled PDF[/dim white]")
        table.add_row("[bold green]2.[/bold green] Open Existing Project", "[dim white]List and inspect previously generated output files[/dim white]")
        table.add_row("[bold green]3.[/bold green] Browse Templates", "[dim white]Explore 25+ professional document structures[/dim white]")
        table.add_row("[bold green]4.[/bold green] Settings", "[dim white]Configure default fonts, margins, page dimensions[/dim white]")
        table.add_row("[bold green]5.[/bold green] Help", "[dim white]Usage documentation, shortcuts, FAQ[/dim white]")
        table.add_row("[bold green]6.[/bold green] About", "[dim white]Mint PDF application context[/dim white]")
        table.add_row("[bold green]7.[/bold green] Exit", "[dim white]Safely quit application[/dim white]")
        
        console.print(Panel(table, title="[bold green]Main Menu[/bold green]", border_style="green", expand=False))
        console.print("\n[dim]At any prompt, type 'b' for Back, 'c' for Cancel, or 'e' to Exit.[/dim]\n")

    def _show_success_panel(self, msg: str) -> None:
        console.print(Panel(f"[bold green]✓ SUCCESS:[/bold green] {msg}", border_style="green", expand=False))

    def _show_warning_panel(self, msg: str) -> None:
        console.print(Panel(f"[bold yellow]! WARNING:[/bold yellow] {msg}", border_style="yellow", expand=False))

    def _show_error_panel(self, msg: str) -> None:
        console.print(Panel(f"[bold red]❌ ERROR:[/bold red] {msg}", border_style="red", expand=False))

    def _show_info_panel(self, msg: str) -> None:
        console.print(Panel(f"[bold cyan]i INFO:[/bold cyan] {msg}", border_style="cyan", expand=False))
    ...
    def create_new_pdf_flow(self) -> None:
        ...
        clear_screen()
        console.print(Panel("[bold green]Step 1 of 12: Accept Text Input[/bold green]", border_style="green", expand=False))
            ...
            try:
                ...
                if method == "1":
                    ...
                    if path.suffix.lower() not in [".txt", ".md", ".docx"]:
                        self._show_error_panel("Unsupported file format. Please use .txt, .md, or .docx.")
                        continue
                    
                    try:
                        content = read_document(path)
                        orig_filename = path.name
                        console.print(f"[green]✓ Successfully loaded {len(content)} characters from {path.name}[/green]")
                        break
                    except Exception as e:
        ...
        clear_screen()
        console.print(Panel("[bold green]Step 2 of 12: Running Document Analyzer[/bold green]", border_style="green", expand=False))
        ...
        rec_table.add_row("[yellow]Recommended Template:[/yellow]", f"[cyan]{analysis.recommended_template}[/cyan]")
        rec_table.add_row("[yellow]Recommended Theme:[/yellow]", f"[cyan]{analysis.recommended_theme}[/cyan]")
        rec_table.add_row("[yellow]Recommended Font:[/yellow]", f"[cyan]{analysis.recommended_font}[/cyan]")
        rec_table.add_row("[yellow]Suggested Filename:[/yellow]", f"[cyan]{analysis.recommended_filename}[/cyan]")
        ...
        clear_screen()
        console.print(Panel("[bold green]Step 3 of 12: Collect Document Metadata[/bold green]", border_style="green", expand=False))
        try:
        ...
        clear_screen()
        console.print(Panel("[bold green]Step 4 of 12: Layout Template Selection[/bold green]", border_style="green", expand=False))
        ...
        clear_screen()
        console.print(Panel("[bold green]Step 5 of 12: Theme Selection[/bold green]", border_style="green", expand=False))
        ...
        try:
            theme_choice = self._prompt(
                "Choose visual theme",
                default=analysis.recommended_theme,
                choices=themes
            )
        ...
        clear_screen()
        console.print(Panel("[bold green]Step 6 of 12: Typography Font Selection[/bold green]", border_style="green", expand=False))
        ...
        try:
            font_choice = self._prompt(
                "Choose font family",
                default=analysis.recommended_font,
                choices=fonts
            )
        except NavigationBack:
            self.create_new_pdf_flow()
        ...
        clear_screen()
        console.print(Panel("[bold green]Step 7 of 12: Page Configuration[/bold green]", border_style="green", expand=False))
        ...
        doc_settings = AppSettings(
            ...
            default_font=font_choice,
            page_size=self.settings.page_size,
            margins=MarginsSettings(**selected_template.margins),
        ...
        console.print(f"  Margins (Left/Right):[cyan]{doc_settings.margins.left}pt / {doc_settings.margins.right}pt[/cyan]")
        console.print(f"  Margins (Top/Bottom):[cyan]{doc_settings.margins.top}pt / {doc_settings.margins.bottom}pt[/cyan]")
        ...
        try:
            ...
            if custom_cfg:
                page_sz = self._prompt("Page size", default=doc_settings.page_size.value, choices=["LETTER", "A4", "LEGAL"])
                doc_settings.page_size = page_sz
                
                top_m = float(self._prompt("Top margin (pt)", default=str(doc_settings.margins.top)))
                bottom_m = float(self._prompt("Bottom margin (pt)", default=str(doc_settings.margins.bottom)))
                left_m = float(self._prompt("Left margin (pt)", default=str(doc_settings.margins.left)))
                right_m = float(self._prompt("Right margin (pt)", default=str(doc_settings.margins.right)))
                
                doc_settings.margins = MarginsSettings(top=top_m, bottom=bottom_m, left=left_m, right=right_m)
                
                auto_pg = Confirm.ask("Enable page numbering?", default=doc_settings.auto_page_numbers, console=console)
                doc_settings.auto_page_numbers = auto_pg
                
                auto_t = Confirm.ask("Enable table of contents?", default=doc_settings.auto_toc, console=console)
        ...
        # Step 8: Cover Page
        clear_screen()
        console.print(Panel("[bold green]Step 8 of 12: Cover Page Setup[/bold green]", border_style="green", expand=False))
        try:
            has_cover = Confirm.ask("Include a professional cover page in the document?", default=True, console=console)
        except NavigationBack:
            self.create_new_pdf_flow()
            return
        ...
        clear_screen()
        console.print(Panel("[bold green]Step 9-11 of 12: Visual Summary & Validation[/bold green]", border_style="green", expand=False))
        ...
        summary_table.add_row("Design Theme", f"{selected_theme.name} (Primary: {selected_theme.primary})")
        summary_table.add_row("Layout Template", f"{selected_template.name} ({selected_template.category})")
        summary_table.add_row("Font Family", font_choice)
        summary_table.add_row("Page Configuration", f"Size: {doc_settings.page_size.value} | Margins: L={doc_settings.margins.left}pt, R={doc_settings.margins.right}pt, T={doc_settings.margins.top}pt, B={doc_settings.margins.bottom}pt")
        ...
        summary_table.add_row("Page Numbers Printed", "Yes" if doc_settings.auto_page_numbers else "No")
        ...
        if final_path.exists():
            ...
            try:
                collision_choice = self._prompt("Select collision recovery option (1-4)", default="2", choices=["1", "2", "3", "4"])
        ...
        clear_screen()
        console.print(Panel("[bold green]Step 12 of 12: Compiling PDF Document[/bold green]", border_style="green", expand=False))
        ...
        if success:
            self._show_success_panel(f"PDF document generated successfully!\nSaved path: [cyan]{final_path}[/cyan]")
        else:
            self._show_error_panel("Failed to compile PDF document. Check logs/mint_pdf.log for detail logs.")
    ...
    def open_existing_project_flow(self) -> None:
        ...
        clear_screen()
        console.print(Panel("[bold green]Open Existing Project[/bold green]", border_style="green", expand=False))
    ...
    def browse_templates_flow(self) -> None:
        ...
        clear_screen()
        console.print(Panel("[bold green]Browse Templates Library[/bold green]", border_style="green", expand=False))
    ...
    def settings_flow(self) -> None:
        ...
        while True:
            clear_screen()
            console.print(Panel("[bold green]Application Settings[/bold green]", border_style="green", expand=False))
            ...
            table.add_row("[green]6.[/green] Default Margins", f"Top={self.settings.margins.top}pt, Bottom={self.settings.margins.bottom}pt, Left={self.settings.margins.left}pt, Right={self.settings.margins.right}pt")
            ...
            table.add_row("[green]9.[/green] Auto Page Numbers", str(self.settings.auto_page_numbers))
            ...
            try:
                ...
                if choice == "1":
                    ...
                    themes = ThemeManager.get_all_theme_names()
                    val = self._prompt("Enter Default Theme", default=self.settings.theme.value, choices=themes)
                    self.settings.theme = val
                    
                elif choice == "3":
                    templates = TemplateManager.get_template_names()
                    val = self._prompt("Enter Default Template", default=self.settings.default_template, choices=templates)
                    self.settings.default_template = val
                    
                elif choice == "4":
                    fonts = FontManager.get_supported_fonts()
                    val = self._prompt("Enter Default Font", default=self.settings.default_font.value, choices=fonts)
                    self.settings.default_font = val
                    
                elif choice == "5":
                    val = self._prompt("Enter Default Page Size", default=self.settings.page_size.value, choices=["LETTER", "A4", "LEGAL"])
                    self.settings.page_size = val
                    
                elif choice == "6":
                    top = float(self._prompt("Top Margin (pt)", default=str(self.settings.margins.top)))
                    bottom = float(self._prompt("Bottom Margin (pt)", default=str(self.settings.margins.bottom)))
                    left = float(self._prompt("Left Margin (pt)", default=str(self.settings.margins.left)))
                    right = float(self._prompt("Right Margin (pt)", default=str(self.settings.margins.right)))
                    self.settings.margins = MarginsSettings(top=top, bottom=bottom, left=left, right=right)
                    ...
                    val = Confirm.ask("Enable Auto TOC by default?", default=self.settings.auto_toc, console=console)
                    self.settings.auto_toc = val
                    
                elif choice == "9":
                    val = Confirm.ask("Enable Auto Page Numbers by default?", default=self.settings.auto_page_numbers, console=console)
                    self.settings.auto_page_numbers = val
    ...
    def help_flow(self) -> None:
        ...
        clear_screen()
        console.print(Panel("[bold green]Help & Documentation[/bold green]", border_style="green", expand=False))
    ...
    def about_flow(self) -> None:
        ...
        console.print(Panel(Align.center(about_text), title="About Mint PDF", border_style="green", expand=False))
        self._wait_for_key()
